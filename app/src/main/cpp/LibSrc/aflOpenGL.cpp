...
INT GLDevice::loadShaders(LPCSTR path)
{
	...
	while((listName = AAssetDir_getNextFileName(dir))!=NULL)
	{
		...
		if(strncmp(listName,"VS_",2)==0)
		{
			...
			//__android_log_print( ANDROID_LOG_FATAL,"SHADER VS","%s\n",fileName.c_str());
		}
		else if(strncmp(listName,"PS_",2)==0)
		{
			addShader(name,GL_FRAGMENT_SHADER,fileName);
			//__android_log_print( ANDROID_LOG_FATAL,"SHADER PS","%s\n",fileName.c_str());
		}
		delete[] name;
...
bool GLShader::onDeviceRestore()
{
	...
	if(compiled == GL_FALSE)
	{
		...
		#ifdef __ANDROID__
		__android_log_print( ANDROID_LOG_DEBUG,"SHADER","[%s]\n%d %s\n",m_fileName.c_str(),length,log);
		#else
...
{
	__android_log_print( ANDROID_LOG_DEBUG,"Render Buffer","%d,%d\n",width,height);
...
bool Texture::open(LPCSTR fileName, bool filter)
{
	...
	__android_log_print(ANDROID_LOG_DEBUG, "Image", "%s", name.c_str());
	...
	__android_log_print(ANDROID_LOG_DEBUG, "Image", "%d,%d\n", textureWidth, textureHeight);
