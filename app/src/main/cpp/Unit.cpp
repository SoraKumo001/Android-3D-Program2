...
void UnitChara::setCommand(COMMAND command, FLOAT param)
{
	m_command |= command;
	m_param[command] = param;
}
...
void UnitChara::onAction(LPVOID value)
{
	...
	if(m_input)
	{
		...
		{
			setCommand(UnitChara::moveFRONT, m_input->getDown(UP));
		}
		else if (m_input->isDown(DOWN))
		{
			setCommand(UnitChara::moveBACK, m_input->getDown(DOWN));
		}
		if (m_input->isDown(LEFT))
		{
			setCommand(UnitChara::moveLEFT, m_input->getDown(LEFT) * 3);
		}
		else if (m_input->isDown(RIGHT))
		{
			setCommand(UnitChara::moveRIGHT, m_input->getDown(RIGHT) * 3);
		}
	...
	if(!(m_stat & statJump || m_stat & statAtack || m_stat & statCrash))
	{
		...
		if(m_command & moveFRONT)
		{
			if(strcmp(animeName,"Walk") != 0)
				setAnimation("Walk",500);
			m_stat |= statMove;
			m_speed = m_param[moveFRONT] * 2.0f;
		}
		else if(m_command & moveBACK)
		{
			...
			if(strcmp(animeName,"Walk") != 0)
				setAnimation("Walk",500);
			m_stat |= statMove;
			m_speed = m_param[moveBACK] * 2.0f;
		}
		if(m_command & moveLEFT)
		{
			setRotationZ(getRotationZ() - m_param[moveLEFT]);
			//setRotationZ(getRotationZ() - 3);
		}
		else if(m_command & moveRIGHT)
		{
			setRotationZ(getRotationZ() + m_param[moveRIGHT]);
			//setRotationZ(getRotationZ() + 3);
		}
